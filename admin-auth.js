...
// =========================================================
// LISTA DE PRODUTOS — RESPONSIVIDADE + FILTROS
// =========================================================
(function configurarListaProdutos() {
    ...
    function ordenarOriginal(linhas, tbody) {
        linhas
            .slice()
            .sort((a, b) => Number(a.dataset.sfOriginal || 0) - Number(b.dataset.sfOriginal || 0))
            .forEach(linha => tbody.appendChild(linha));
    }
    ...
    function aplicarFiltros() {
        ...
        // Guarda a posição original somente uma vez para cada linha.
        linhas.forEach((linha, index) => {
            if (!linha.dataset.sfOriginal) linha.dataset.sfOriginal = String(index);
        });
        ...
        linhas.forEach(linha => {
            ...
            const atendeNome = !termo || nome.includes(termo);
            const atendeCategoria = !cat || cat === 'todas' || categoriaLinha === cat;
            linha.hidden = !(atendeNome && atendeCategoria);
        });
        ...
        if (ordem?.value === 'az' || ordem?.value === 'za') {
            linhas
                ...
                .sort((a, b) => {
                    ...
                    return ordem.value === 'az' ? resultado : -resultado;
                })
                .forEach(linha => tbody.appendChild(linha));
        } else {
            ordenarOriginal(linhas, tbody);
        }
    }

    function ligarEventos() {
        // Delegação: funciona mesmo que o admin.js substitua o conteúdo da lista.
        if (document.documentElement.dataset.filtrosProdutosLigados === '1') return;
        document.documentElement.dataset.filtrosProdutosLigados = '1';
        ...
        document.addEventListener('change', (evento) => {
            if (evento.target?.id === 'filtro-categoria' || evento.target?.id === 'filtro-ordem') aplicarFiltros();
        });

        // O admin.js usa onSnapshot e recria as linhas; reaplica os filtros depois disso.
        const observer = new MutationObserver(() => {
            clearTimeout(window.__sfFiltroTimer);
            window.__sfFiltroTimer = setTimeout(aplicarFiltros, 20);
        });

        const iniciarObserver = () => {
            const tbody = document.getElementById('lista-produtos-body');
            if (tbody && !tbody.dataset.sfObserver) {
        ...
        // Garante ligação caso o elemento só apareça depois do carregamento do Firestore.
        setInterval(() => {
            const tbody = document.getElementById('lista-produtos-body');
            if (tbody && !tbody.dataset.sfObserver) {
                observer.observe(tbody, { childList: true });
